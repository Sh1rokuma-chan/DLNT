...
http://localhost/         → master-nginx
  ├─ /                   → Open WebUI  (汎用チャット)
  └─ /agent/             → Agent UI v3 (3エージェント専用ダッシュボード)
...
| **Agent UI v3** | 3エージェント専用カスタムダッシュボード |
...
├── agent-ui/                # Agent UI v3 (Express + カスタムHTML)
...
Agent UI を v3 として再設計し、以下を改善しました。

- **2カラム情報設計**: 左サイドバーにエージェント切替・システム状態を集約
- **操作導線の整理**: 会話・ログ・入力をカード化し、視線移動を最小化
- **保守性向上**: `index.html` のインライン実装を `styles.css` / `app.js` に分離
- **レスポンシブ対応**: 狭い画面ではサイドバーとログを縦積み表示
...
| http://localhost/agent/ | Agent UI v3 (調査・タスク・議事録) |
