...
http://localhost/         → master-nginx
  ├─ /                   → Open WebUI  (汎用チャット)
  └─ /agent/             → Agent UI v2 (3エージェント専用ダッシュボード / v2移行中)
...
├── api/                     # FastAPI最小構成（v2向け）
│   ├── app/
│   │   ├── main.py          # FastAPIエントリポイント
│   │   └── config.py        # 設定読み込み
│   ├── pyproject.toml       # Python依存管理
│   └── Dockerfile           # APIコンテナ定義
├── frontend/                # v2フロントエンド用（移行準備）
├── db/                      # DB初期化・マイグレーション定義
│   ├── init.sql             # 初期化SQL
│   ├── alembic.ini          # Alembic設定
│   └── alembic/
│       ├── env.py           # Alembic実行環境
│       └── versions/        # マイグレーションファイル配置先
├── config/                  # 環境別設定テンプレート
│   ├── app.example.env
│   └── docker.example.env
...
├── agent-ui/                # Agent UI v2 (Express + カスタムHTML / v2移行中のため当面維持)
